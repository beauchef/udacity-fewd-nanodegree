...
// Replaced 2 functions with switch cases with this object
var pizzaSliderInfo = {
  "1": {
    "size": 25,
    "label": "Small"
...
var resizePizzas = function(size) {
  ...
  document.querySelector("#pizzaSize").innerHTML = pizzaSliderInfo[size].label;
  for (var i = 0; i < pizzaContainers.length; i++) {
    pizzaContainers[i].style.width = pizzaSliderInfo[size].size +'%';
  }
...
/**
 * Function to create the pizzas when the page loads.
 */
function initPizzas() {
 ...
 *
 * @param times array of User Timing measurements from updatePositions()
 */
...
function updatePositions() {
  ...
  // pre-calculate phases, following suggestion in forums :)
  var phase = [];
  for (var i = 0; i < 5; i++) {
    phase[i] = Math.sin(scrollTop + i) * 100;
  }

  for (var i = 0; i < pizzaMovers.length; i++) {
    pizzaMovers[i].style.left = pizzaMovers[i].basicLeft + phase[i % 5] + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  pizzaMovers = document.querySelectorAll('.mover');
